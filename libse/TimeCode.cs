...
namespace Nikse.SubtitleEdit.Core
{
    public class TimeCode
    {
        ...
        public string ToHHMMSSFF(double framerate=23.97d)
        {
            string s;
            var ts = TimeSpan;
        ...
        public string ToSSFF(double framerate = 23.97d)
        {
            string s;
            var ts = TimeSpan;
            var frames = Math.Round(ts.Milliseconds / (BaseUnit / framerate));
        ...
        public string ToHHMMSSPeriodFF(double framerate = 23.97d)
        {
            string s;
            var ts = TimeSpan;
            var frames = Math.Round(ts.Milliseconds / (BaseUnit / framerate));
        ...
        public string ToDisplayString(double framerate = 23.97d, bool useHHMMSSFF = true)
        {
            if (IsMaxTime)
            {
                return "-";
            ...
            return useHHMMSSFF ? ToHHMMSSFF(framerate) : ToString(true);
        }

        public string ToShortDisplayString(bool useHHMMSSFF = true)
        {
            if (IsMaxTime)
            {
                return "-";
            ...
            return useHHMMSSFF == true ? ToShortStringHHMMSSFF() : ToShortString(true);
        }
